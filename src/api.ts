...
const DEFAULT_API = "https://api-d.adey.lol"; // local dev
...
export async function createPayment(opts: {
  ...
  if (!res.ok) {
    const errMsg =
      json?.error ||
      JSON.stringify(json) ||
      `createPayment failed (${res.status})`;
...
export async function getPaymentStatus(
  ...
  const res = await fetch(
    `${localApiBase}/payment-status/${encodeURIComponent(paymentId)}`
  );
  if (!res.ok) throw new Error(`status fetch failed (${res.status})`);
  return res.json();
}
// src/api.ts (add)
export async function requestPayoutByEmail(opts: {
  amount: number;
  toEmail: string;
  note?: string;
  apiKey?: string;
  idempotencyKey?: string;
}): Promise<{ payoutId: string; txnRef?: string }> {
  const apiKey = opts.apiKey ?? globalApiKey;
  const apiKeyStr = ensureString("apiKey", apiKey);
  ...
  if (!opts.toEmail || typeof opts.toEmail !== "string") {
  ...
  const res = await fetch(`${localApiBase}/merchant/payout-email`, {
    method: "POST",
    headers: headers as HeadersInit,
    body: JSON.stringify({
      amount: opts.amount,
  ...
  const json = await res.json().catch(() => ({}));
  if (!res.ok) {
    const err = json?.error || JSON.stringify(json) || `Payout failed (${res.status})`;
    throw new Error(err);
  }
  return json as { payoutId: string; txnRef?: string };
}
